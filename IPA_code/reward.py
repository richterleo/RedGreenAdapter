...
class Reward:
    ...
    '''
    Toxicity reward used during RL
    '''
    def __init__(self, save_path: str, batch_size: int, device: int, params: argparse.Namespace):
        ...
        self.toxicity_classifier = pipeline("text-classification", model=self.tox_model_name)

    def get_reward(self, prompts: List[str], responses: List[str]) -> Dict[str, List[float]]:
        
        texts = [f"{prompt} {response}" for prompt, response in zip(prompts, responses)]  
        results = self.toxicity_classifier(texts)
        tox_scores = [result['score'] for result in results if result['label'] == 'LABEL_1']  # Assuming 'LABEL_1' indicates toxicity
